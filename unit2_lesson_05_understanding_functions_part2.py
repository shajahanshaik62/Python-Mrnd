...
# parameters with default s allows you to write one api without having a large number
...
def test_function_call_with_keyword_arguments():
    ...
    assert [143, 20, 3, 4] == demo(143,second=20)
    assert [143,20,30,4] == demo(143,second=20, third=30)
    assert [10,143, 30,4] == demo(first=10, second=143 ,third=30)
    assert [10,143,30,4] == demo(10, second=143, third=30)
    assert [10, 20, 10, 4] == demo(10, second=20, third=10)  # is this allowed? correct and uncomment
...
def simple_format(format, *args):
    ...
    for s in range(len(args)):
        format=format.replace(("%"+str(s)), args[s])
    return  format
...
-clearly unerstood the difference between parameter and argument
